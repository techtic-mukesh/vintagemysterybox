!(function (e, t) {
    ...
    // Store original cart total (without shipping) for calculations
    var originalCartTotal = 0;
    var cartFinalElement = document.querySelector('#csFinal');
    
    if (cartFinalElement) {
        // Extract numeric value from the cart total (remove currency symbols and formatting)
        var cartText = cartFinalElement.textContent || cartFinalElement.innerText;
        console.log('Raw cart text:', cartText);
        
        // Remove currency symbols and extract the number properly
        var numericValue = cartText.replace(/[£$€¥₹₽]/g, '').replace(/[^\d.]/g, '');
        console.log('Numeric value extracted:', numericValue);
        
        originalCartTotal = Math.round(parseFloat(numericValue) * 100); // Convert to cents/pence
        console.log('Original cart total in pence:', originalCartTotal);
    }
    ...
    // Function to update cart shipping price and total
     function updateCartShipping(price) {
        console.log('=== SHIPPING CALCULATION DEBUG ===');
        console.log('1. Original cart total (pence):', originalCartTotal);
        console.log('2. Shipping price parameter:', price, '(type:', typeof price, ')');
        
        var cartShippingElement = document.querySelector('#cartShipping');
        var cartFinalElement = document.querySelector('#csFinal');
        
        if (cartShippingElement) {
            var formattedPrice = T4SThemeSP.Currency.formatMoney(price);
            console.log('3. Formatted shipping price:', formattedPrice);

            // Check if the formatted price is zero
            if (formattedPrice === '£0.00') {
                formattedPrice = 'FREE';
            }

            cartShippingElement.innerHTML = formattedPrice;
            }

        
        // Update cart total by adding shipping to original cart total
                if (cartFinalElement) {
                console.log('=== SHIPPING CALCULATION DEBUG ===');

                // Remove currency symbols, spaces etc. and normalize decimal separator
                var cleanPrice = price.replace(/[^\d.,]/g, '').replace(',', '.');
                console.log('2. Clean shipping price:', cleanPrice);

                // Convert shipping to cents (major units × 100)
                var shippingInCents = Math.round(parseFloat(cleanPrice) * 100) || 0;
                console.log('3. Shipping converted to int (cents):', shippingInCents);

                // ✅ Normalize originalCartTotal
                // If it's suspiciously big (already multiplied twice), scale it back
                if (originalCartTotal > 10000) {
                    console.log('⚠️ originalCartTotal too large, normalizing...');
                    originalCartTotal = Math.round(originalCartTotal / 100);
                }
                console.log('4. Original cart total (cents):', originalCartTotal);

                // Add cart total + shipping
                var newTotal = originalCartTotal + shippingInCents;
                console.log('5. Calculation:', originalCartTotal, '+', shippingInCents, '=', newTotal);

                // Format in store’s active currency
                var formattedTotal = T4SThemeSP.Currency.formatMoney(newTotal);
                console.log('6. Final formatted total:', formattedTotal);

                // cartFinalElement.innerHTML = formattedTotal;
    ...
    t.on("click", '[data-action="estimate-shipping"]', function (tEvent) {
        ...
        btn.addClass(N.loading);
        document.dispatchEvent(new CustomEvent("theme:loading:start"));

        formData.country = container.find('[name="country"]').val() || "";
        formData.province = container.find('[name="province"]').val() || "";
        formData.zip = container.find('[name="zip"]').val() || "";

        fetch(H + "/shipping_rates.json?" + e.param({ shipping_address: formData }), {
            credentials: "same-origin",
            method: "GET"
        }).then(function (res) {
            document.dispatchEvent(new CustomEvent("theme:loading:end"));
            btn.removeClass(N.loading);

            res.json().then(function (json) {
                console.log("📬 Shipping API Response JSON:", json);
                ...
                (function (ok, data, formData, langRates) {
                    if (ok) {
                        // SUCCESS
                        var addressStr = "",
                            message = "",
                        ...
                        if (formData.zip) addressStr += formData.zip + ", ";
                        if (formData.province) addressStr += formData.province + ", ";
                        addressStr += formData.country;

                        if (data.shipping_rates.length > 1) {
                            var firstPrice = T4SThemeSP.Currency.formatMoney(data.shipping_rates[0].price);
                            message = langRates.multiple_rates
                                .replace("[number_of_rates]", data.shipping_rates.length)
                                .replace("[address]", addressStr)
                                .replace("[rate]", firstPrice);
                        } else {
                            message = data.shipping_rates.length === 1
                                ? langRates.one_rate.replace("[address]", addressStr)
                                : langRates.no_rates;
                        }

                        // Generate radio button list items with descriptions
                        function generateShippingRadioItems(shippingRates) {
                            let radioItems = "";
                            
                            e.each(shippingRates, function (i, rate) {
                                console.log('Shipping rate data:', rate);
                                console.log('Raw price from API:', rate.price);
                                console.log('Parsed price:', rawPrice);
                                
                                var rawPrice = parseInt(rate.price, 10);
                                var formattedPrice = T4SThemeSP.Currency.formatMoney(rate.price);
                                var value = rate.code || rate.name.toLowerCase().replace(/\s+/g, '_');
                                
                                // Build description from available data
                                var description = "";
                                if (rate.description) {
                                    description = rate.description;
                                } else if (rate.delivery_days && rate.delivery_days.length >= 2) {
                                    description = `${rate.delivery_days[0]} to ${rate.delivery_days[1]} business days`;
                                } else if (rate.delivery_range) {
                                    description = rate.delivery_range;
                                }
                                
                                // Display "FREE" if price is 0, otherwise show formatted price
                                var priceDisplay = rawPrice === 0 ? "FREE" : formattedPrice;
                                ...
                                radioItems += `
                                    <li>
                                        <label>
                                            <input type="radio" name="shipping_method" value="${value}" data-price="${rate.price}" ${checkedAttr}>
                                            <span class="shipping-option">
                                                <span class="shipping-name">${rate.name}</span>
                                               
                                                ${description ? `<span class="shipping-description">${description}</span>` : ''}
                                            </span>
                        ...
                        const radioListItems = generateShippingRadioItems(data.shipping_rates);


                        // Append the static option to the existing list
                        var successHTML =
                            '<div class="t4s-mess__rates is--rates-success">' +
                            message +
                            '</div><div class="t4s-results__rates"><ul class="t4s-shipping-rates-list">' +
                            radioListItems +
                            '</ul></div>';

                        shippingMethod.innerHTML = successHTML;


                        // Update cart shipping with first option price (if available)
                        if (data.shipping_rates.length > 0) {
                            updateCartShipping(data.shipping_rates[0].price);
                        }

                        // Add event listener for radio button changes
                      var radioButtons = shippingMethod.querySelectorAll('input[name="shipping_method"]');
                        radioButtons.forEach(function(radio) {
                            radio.addEventListener('change', function () {
                                if (this.checked) {
                                    var selectedPrice = this.getAttribute('data-price');
                                    var shippingTitle = this.value; // You may also use a `data-title` attribute if preferred

                                    updateCartShipping(selectedPrice);

                                    // Update hidden input value
                                    var hiddenInput = document.querySelector('input[name="shippingTitle"]');
                                    if (hiddenInput) {
                                        hiddenInput.value = shippingTitle;
                                    }
                                }
                            });
                        });
                        ...
                        Object.keys(data).forEach(function (key) {
                            errorsHTML += "<li><span class='t4s-key__rate'>" + key + "</span> " + data[key] + "</li>";
                        });

                        if (errorsHTML === "country is not supported.") {
                            errorsHTML = `<li>${langRates.no_rates}</li>`;
                        }

                        var errorHTML =
                            `<p>${langRates.errors}</p><ul class="t4s-mess__rates is--rates-error">${errorsHTML}</ul>`;
                        shippingMethod.innerHTML = errorHTML;
                    }

                    // Optional: scroll to the shipping method area after update
                    // shippingMethod.scrollIntoView({ behavior: "smooth" });

                })(res.ok, json, formData, btn[0].langRates);
            });
        });
    }.bind(t[0]));
}
            ...
            (T4SThemeSP.Wishlist = (function () {
                                ...
                                : (function (t) {
                                      ...
                                      t.addClass(U + " " + P),
                                          fetch(B, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ product_id: n, product_handle: i, action: "add" }) })
                                              ...
                                              .then(function (t) {
                                                  "success" == t.status
                                                      ? ((q = JSON.parse(t.response.metafield.value).the4_ids),
                                                        (H = JSON.parse(t.response.metafield.value).ecomrise_ids),
                                                        q && (H = H.concat(q)),
                ...
                function o(t) {
                    ...
                    e(w + '[data-id="' + n + '"]')
                        ...
                        e(C).html(E),
                        r(),
                        W && ("" == N.toString() || z) && (e(".t4s-products-wishlist").fadeTo(300, 0), (window.location.href = O));
                }
                function a(t) {
                    var n = t.attr("data-id") || "",
                        i = t.attr("data-handle") || "ntt4s" + n;
                    t.addClass(U + " " + P),
                        fetch(B, {
                            ...
                            body: JSON.stringify({ product_id: n, product_handle: i, namespace: q.indexOf(n) >= 0 ? "the4" : "", action: "add", _method: "DELETE" }),
                        })
                            .then(function (e) {
                                return e.json();
                            })
                            .then(function (i) {
                                if ("success" == i.status) {
                                    ...
                                    e(w + '[data-id="' + n + '"]')
                                        ...
                                        e(C).html(E),
                                        l(),
                                        W && ("" == N.toString() || z) && (e(".t4s-products-wishlist").fadeTo(300, 0), (window.location.href = R));
        ...
        (T4SThemeSP.PopupPro = (function () {
            ...
            function d() {
                I.$popup.removeClass("on--hide").addClass("on--show"),
                    I.$popup.on(I.click, ".t4s-pp_cookies__accept-btn", function (e) {
                        e.preventDefault(), D && CookiesT4.set(I.CookiesName, "accepted", { expires: I.day_next, path: "/" }), window.Shopify.customerPrivacy.setTrackingConsent(!0, u);
                    }),
                    I.$popup.on(I.click, ".t4s-pp_cookies__decline-btn", function (e) {
                        e.preventDefault(), D && CookiesT4.set(I.CookiesName, "accepted", { expires: I.day_next, path: "/" }), window.Shopify.customerPrivacy.setTrackingConsent(!1, u);
                    });
            }
            function u() {
            ...
            (T4SThemeSP.PopupFetch = function () {
                ...
                b && (t += location.href.indexOf("customer_posted=true") > -1 ? "&" + location.href.split("?")[1] : ""),
